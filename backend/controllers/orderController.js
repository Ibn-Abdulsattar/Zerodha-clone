...
export const createOrder = wrapAsync(async (req, res) => {
  ...
  if (newOrder.qty < 1 || newOrder.price < 1 || !highFund || highFund.availableMargin < totalCost) {
    throw new ExpressError('Invalid quantity or insufficient margin', 400);
  }

  if (newOrder.mode === 'Sell' && newOrder.qty > availabelQty) {
    throw new ExpressError('Not enough stock to sell!', 400);
  }
...
// Get All Orders
export const allOrder = wrapAsync(async (req, res) => {

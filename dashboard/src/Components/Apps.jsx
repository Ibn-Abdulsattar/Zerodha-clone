...
const Apps = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8080/userApp/user-apps")
...
const handleToggle = async (appId, action) => {
  ...
  await axios.post(`http://localhost:8080/userApp/${endpoint}`, { appId });

  const res = await axios.get("http://localhost:8080/userApp/user-apps");
  setApps(res.data);
};
